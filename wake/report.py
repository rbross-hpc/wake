...
def bake_markdown(
    ...
    *base* (the wake-out work-dir root) is optional and used only to
        ...
        if td.exists():
            for p in sorted(td.glob("*.json")):
                try:
                    ...
                    continue
                if theme.get("theme_status") == "confirmed":
                    themes_confirmed += 1
                else:
                    themes_draft += 1
            themes_exist = (themes_confirmed + themes_draft) > 0
    ...
    lines.append(f"seed_openalex_id: {oid}")
    if doi:
        lines.append(f"seed_doi: {doi}")
    if year:
        lines.append(f"seed_year: {year}")
    if authors:
        author_list = ", ".join(f'"{a}"' for a in authors[:8])
        lines.append(f"seed_authors: [{author_list}]")
    lines.append(f"citing_count: {metrics.get('total_citing_works', 0)}")
    lines.append(f"verified_count: {metrics.get('verified_count', 0)}")
    provisional_count = metrics.get("classified_count", 0) - metrics.get("verified_count", 0)
    lines.append(f"provisional_count: {provisional_count}")
    ...
    nav_links = []
    if evidence_exists:
        nav_links.append("[full evidence wiki](evidence/index.md)")
    if themes_exist:
...
def bake_and_save(
    ...
    *citing_works* may be the full set (some possibly unclassified — the
    ...
    md_text = bake_markdown(seed_work, metrics, base=base)
